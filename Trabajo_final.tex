...
Como primer paso, nos aseguraríamos de tener instalada la librería de
tidyverse, después, cargaríamos los datos copiando su respectiva ruta.
Luego, replicamos la exploración de datos que se aplicó en el dashboard
en la línea 36 de ``procesamiento\_datos.py'', donde se agrupa por
\textbf{location} y se calcula el promedio y el conteo de
\textbf{salary\_usd}. En R, usamos el operador \emph{\%\textgreater\%}
para encadenar estas acciones.

Lo siguiente sería la limpieza de habilidades (\textbf{skills\_copy}),
Esta es la parte más importante para que no falten datos. en el
dashboard se separan las habilidades por comas y crea una fila por cada
una. En R, la función mágica para esto es \emph{separate\_rows()}.

Ya con estos pasos principales, podriamos reflejar qué datos faltantes
tenemos en nuestra data, y en el caso de tenerlos, saber cómo proceder
...
## 4 location                  0          0
## 5 salary_usd                0          0
## 6 skills_required           0          0
## 7 remote_option             0          0
## 8 company_size              0          0
## 9 posting_date              0          0
\end{verbatim}

Como no hay nada en la data que represente algún problema, siendo una
data perfecta y limpia para trabajar con todos los datos, podemos
